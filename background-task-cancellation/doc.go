...
As simple, and useless, the `mockComplexOp` is, it represents a real-world scenario where we need
...
One thing to notice, when we spawn a background goroutine, the Go runtime will take ownership of that task
...
Of course there are simpler ways of tracking goroutines completion in go (for instance using `sync.WaitGroup`), but
it's good to understand how we can do this by ourselves using channels.
...
The first thing this function does is **decoupling the long-running operation from the wrapper**. Because the operation is
uninterruptible, we can't handle cancellation if the routine is busy running the operation.

We use the `go` keyword to spawn a new sub-routine and delegate the operation execution to it. Because this is an *inner
...
In this implementation, if the `cancelCh` is closed before the operation sub-routine can complete, the `op.Stop()` method
is called to request graceful termination.
...
As we can see the implementation is almost identical, except for the cancellation `case` in the handling logic.
